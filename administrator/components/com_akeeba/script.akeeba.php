...
class Com_AkeebaInstallerScript extends F0FUtilsInstallscript
{
	...
	public function uninstall($parent)
	{
		...
		{
			// Joomla! 2.5.0 or later; we have to use the Quickicon plugin so we shouldn't install the icon module
			$key = array_search('akeebabackup', $this->uninstallation_queue['modules']['admin']);
			unset($this->uninstallation_queue['modules']['admin'][$key]);
		}
		else
		{
			// Joomla! 1.x; we have to use the icon module so we shouldn't install the quickicon plugin
			$key = array_search('akeebabackup', $this->uninstallation_queue['plugins']['quickicon']);
			unset($this->uninstallation_queue['plugins']['quickicon'][$key]);
		}

		parent::uninstall($parent); // TODO: Change the autogenerated stub
	}
	...
	function postflight($type, $parent)
	{
		...
		{
			// Joomla! 2.5.0 or later; we have to use the Quickicon plugin so we shouldn't install the icon module
			unset($this->installation_queue['modules']['admin']['akeebabackup']);
		}
		else
		{
			// Joomla! 1.x; we have to use the icon module so we shouldn't install the quickicon plugin
			unset($this->installation_queue['plugins']['quickicon']['akeebabackup']);
		}

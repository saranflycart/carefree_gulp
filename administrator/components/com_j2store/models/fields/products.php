...
*/
		...
		$script[] = '		jQuery("#modalProduct' . $this->id . '").modal("hide");';
		...
		if ($allowClear && !$scriptClear)
		{
			...
			$script[] = '		document.getElementById(id + "_name").value = "' .
				htmlspecialchars(JText::_('COM_J2STORE_SELECT_AN_ARTICLE', true), ENT_COMPAT, 'UTF-8') . '";';
		...
		$html = array();
		$link = 'index.php?option=com_j2store&amp;view=products&amp;layout=modal&amp;tmpl=component&amp;function=jSelectProduct_' . $this->id;
		...
		{
			$db    = JFactory::getDbo();
			$query = $db->getQuery(true)->select('#__content.title')->from('#__j2store_products');
			...
			$query->where('#__j2store_products.product_source ='.$db->q('com_content'));
			$query->where('#__j2store_products.j2store_product_id = '. (int) $this->value);
		...
		if (empty($title))
		{
			$title = JText::_('COM_J2STORE_SELECT_AN_ARTICLE');
		}
		...
		$html[] = '<a href="#modalProduct' . $this->id . '" class="btn hasTooltip" role="button"  data-toggle="modal" title="'
			. JHtml::tooltipText('COM_J2STORE_CHANGE_PRODUCT') . '">'
		...
		if ($allowClear)
		{
			$html[] = '<button id="' . $this->id . '_clear" class="btn' . ($value ? '' : ' hidden') . '" onclick="return jClearProduct(\'' .
		...
		$html[] = JHtml::_(
			...
			'modalProduct' . $this->id,
			array(
				'url' => $url,
				'title' => JText::_('COM_J2STORE_CHANGE_PRODUCT'),
				'width' => '800px',
				'height' => '300px',

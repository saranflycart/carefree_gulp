...
class plgSystemJ2rsevents extends JPlugin {
	...
	public function onJ2StoreBeforeDisplaySingleProductOption($product,&$option){
		$show_option = $this->params->get('show_options',1);
		if(!$show_option){
			$option['type'] = '';
		}
	}
	...
	public function rsepro_addMenuOption() {
		...
		var joomla_version  = {$joomla_version};
		...
		if(joomla_version == 1){
			//for joomla 3.5
			form.append('<script src=\"{$root}/media/media/js/mediafield.min.js\" type=\"text\/javascript\"><\/script>');			
		}
	...
	public function rsepro_afterEventStore($table){
		...
		if(isset( $inp_data['jform']['j2rsform']['j2store'] ) && !empty( $inp_data['jform']['j2rsform']['j2store'] )){
			...
			F0FModel::getTmpInstance('Products', 'J2StoreModel')->save($j2store_attribs_obj);
		}
		return true;
	}
